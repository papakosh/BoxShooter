...
public class GameManager : MonoBehaviour {
	...
	void Start () {
		...
		if (canBeatLevel) {
			mainScoreDisplay.text = "0 of " + beatLevelScore.ToString ();
		} else {
	...
	void Update () {
		//Debug.Log ("players health points are " + playerHealth.healthPoints);
		if (!gameIsOver) {
			if (canBeatLevel && (score >= beatLevelScore)) {  // check to see if beat game
				...
				AudioSource.PlayClipAtPoint (playerDeathSFX, player.transform.position, 5.0f);
				EndGame ();
